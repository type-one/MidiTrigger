...
ParseAndTrigger::ParseAndTrigger(const std::string& p_name)
    : m_stop(false)
{
...
ParseAndTrigger::~ParseAndTrigger() { teardown(); }

void ParseAndTrigger::setup()
{
...
void ParseAndTrigger::process_message(double /*delta_time*/, const std::vector<unsigned char>& message)
{
    std::string label;
...
void ParseAndTrigger::stop() { m_stop.store(true); }

void ParseAndTrigger::midi_cb(double delta_time, std::vector<unsigned char>* message, void* user_data)
{
